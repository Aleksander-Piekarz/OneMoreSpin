...
const RegisterPage: React.FC = () => {
  ...
  const validate = () => {
    ...
    if (surname.trim().length < 2) {
      e.surname = "Surname must be at least 2 characters";
    }
    if (!birthDate) {
      ...
      if (currentYear - birthYear < 18) {
        e.birthDate = "You must be at least 18 years old";
      }
    }
    ...
    if (!agreeTerms) {
      e.agreeTerms = "You must accept the terms of use";
    }
    if (!agreeAge) {
      e.agreeAge = "You must confirm you are 18 years old";
    }
